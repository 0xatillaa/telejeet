    ...
    for (const chat of publicSearchRes.chat_ids) {
      try {
        ...
        if (chatInfo.type._ == "chatTypeSupergroup") {
          const superGroupResults = await client.invoke({
            _: "getSupergroup",
            supergroup_id: chatInfo.type.supergroup_id,
          });
          if (
            superGroupResults.status._ == "chatMemberStatusRestricted" ||
            superGroupResults.status._ == "chatMemberStatusBanned"
          ) {
            // leave this shitty ass group
            console.log("leave this group:", chatInfo.title);
            continue;
          }

          if (
            !chatInfo.type.is_channel &&
            chatInfo.permissions.can_send_other_messages
          ) {
            //spam
